#!/usr/bin/env bash
# =============================================================================
# NCAE Cyber Games 2026 - Shell / SMB Firewall Lockdown
#
# PURPOSE:
#   Apply shell/SMB network restrictions only after the real topology is known.
...
echo "[$(date)] === Shell/SMB Firewall START ==="
...
# Preserve admin SSH from the confirmed management path.
...
# Preserve scoring/external service access.
firewall-cmd --permanent --zone=ncae-shell \
    --add-rich-rule="rule family='ipv4' source address='${SCORING_NET}' service name='ssh' accept" 2>/dev/null || true
firewall-cmd --permanent --zone=ncae-shell \
    --add-rich-rule="rule family='ipv4' source address='${SCORING_NET}' service name='samba' accept" 2>/dev/null || true
...
echo "  SMB allowed from       : ${SCORING_NET}"
echo "[$(date)] === Shell/SMB Firewall COMPLETE ==="
